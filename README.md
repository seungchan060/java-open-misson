...
```mermaid
flowchart LR
    A[GameApplication<br/>main()] --> B[BattleEngine]

    B --> C[InputView<br/>/ OutputView]
    B --> D[Board & Position]
    B --> E[Units<br/>Role / Stats / TeamSide]
    B --> F[Rules<br/>VictoryRule / FatigueRule]
    B --> G[Services]

    subgraph Services
        G1[MovementService]
        G2[AttackService]
        G3[EnemyAiService<br/>+ ThreatPolicy]
        G4[RecruitService]
    end

    E --> H[Skills<br/>Skill 인터페이스<br/>Backstab / SmokeBomb / Taunt / BodyBlock / Shield / Valor]
...
- `GameApplication` 은 진입점이며,
    
    `Board`, 초기 `Unit` 목록, `InputView`, `OutputView` 를 조합해서 `BattleEngine` 을 만든다.
    
- `BattleEngine` 은
    - 매 턴마다 View를 통해 CLI 입출력을 처리하고,
    - `MovementService`, `AttackService`, `EnemyAiService`, `RecruitService`, `VictoryRule`, `FatigueRule` 을 사용해
        
        실제 게임 한 턴을 진행한다.
        
- `Unit` 은 `Role`, `Stats`, `TeamSide`, `Position` 등을 포함한 도메인 객체이며,
    
    `Skill` 들이 여기에 붙어서 스킬 로직을 수행한다.
    
- `EnemyAiService` 는 `ThreatPolicy` 를 사용해 공격 타겟을 고르고,
    
    `MovementService` 와 `AttackService` 를 호출해 실제 행동을 수행한다.